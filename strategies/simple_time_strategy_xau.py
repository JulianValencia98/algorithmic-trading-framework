...
class SimpleTimeStrategyXAU(StrategyBase):
    """
    Strategy for XAUUSD that opens a buy position, keeps it open for 20 minutes,
    closes it, and immediately opens another buy.
    Magic Number: 3
    
    Risk Management:
    - Fixed lot size: 0.01 (smaller for gold due to volatility)
    - Fixed SL: 100 pips
    - Fixed TP: 300 pips
    """
    ...
    def __init__(self):
        super().__init__()
        self.magic_number = 3  # Unique identifier for this strategy
        self.last_signal_time = None
        self.position_open_time = None
        self.has_position = False
        
        # Strategy-specific risk parameters
        self.fixed_lot = 0.01  # Smaller lot for gold
        self.sl_pips = 100.0
        self.tp_pips = 300.0
    ...
    def get_parameters(self) -> dict:
        return {
            'strategy_name': 'SimpleTimeStrategyXAU',
            'magic_number': self.magic_number,
            'description': 'Opens buy position on XAUUSD, waits 20 min, closes and reopens',
